...
description: "System::IO::BasicSTDOStreamWrapper::Read 方法。如果包装模式为二进制，则从流中读取指定数量的字节；否则读取指定数量的字符并将其转换为 uint8_t 类型。将读取结果写入指定的字节数组。此功能在 C++ 中不受支持！"
type: docs
weight: 400
...
如果包装模式为二进制，则从流中读取指定数量的字节；否则读取指定数量的字符并将其转换为 uint8_t 类型。将读取结果写入指定的字节数组。不支持！

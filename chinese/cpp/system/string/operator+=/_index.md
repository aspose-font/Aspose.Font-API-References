---
title: "System::String::operator+= 方法"
linktitle: "operator+="
second_title: "Aspose.Font 适用于 C++"
description: "System::String::operator+= 方法。C++ 中的连接赋值运算符。"
type: docs
weight: 2900
url: /zh/cpp/system/string/operator+=/
---
## String::operator+=(char_t) method


连接赋值运算符。

```cpp
String & System::String::operator+=(char_t c)
```


| 参数 | 类型 | 描述 |
| --- | --- | --- |
| c | char_t | 要添加到当前字符串末尾的字符。 |

### ReturnValue

自身引用。

## 另见

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::operator+=(const String\&) method


连接赋值运算符。

```cpp
String & System::String::operator+=(const String &str)
```


| 参数 | 类型 | 描述 |
| --- | --- | --- |
| str | const String\& | [String](../) 用于添加到当前字符串的末尾。 |

### ReturnValue

自身引用。

## 另见

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::operator+=(double) method


连接赋值运算符。

```cpp
String & System::String::operator+=(double value)
```


| 参数 | 类型 | 描述 |
| --- | --- | --- |
| value | double | [Double](../../double/) 用于添加到当前字符串的末尾。 |

### ReturnValue

自身引用。

## 另见

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::operator+=(int16_t) method


连接赋值运算符。

```cpp
String & System::String::operator+=(int16_t value)
```


| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 值 | int16_t | Short 用于添加到当前字符串的末尾。 |

### ReturnValue

自身引用。

## 另见

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::operator+=(int32_t) method


连接赋值运算符。

```cpp
String & System::String::operator+=(int32_t value)
```


| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 值 | int32_t | Int 用于添加到当前字符串的末尾。 |

### ReturnValue

自身引用。

## 另见

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::operator+=(int64_t) method


连接赋值运算符。

```cpp
String & System::String::operator+=(int64_t value)
```


| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 值 | int64_t | Long 用于添加到当前字符串的末尾。 |

### ReturnValue

自身引用。

## 另见

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::operator+=(T) method


连接赋值运算符。

```cpp
template<typename T,std::enable_if_t< std::is_same< T, bool >::value > *> String & System::String::operator+=(T value)
```


| 参数 | 描述 |
| --- | --- |
| T | 用于与字符串连接的值类型。必须是 bool。 |

| 参数 | 类型 | 描述 |
| --- | --- | --- |
| value | T | [Boolean](../../boolean/) 用于添加到当前字符串的末尾。 |

### ReturnValue

自身引用。

## 另见

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::operator+=(uint16_t) method


连接赋值运算符。

```cpp
String & System::String::operator+=(uint16_t value)
```


| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 值 | uint16_t | Unsigned short 用于添加到当前字符串的末尾。 |

### ReturnValue

自身引用。

## 另见

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::operator+=(uint32_t) method


连接赋值运算符。

```cpp
String & System::String::operator+=(uint32_t value)
```


| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 值 | uint32_t | Unsigned int 用于添加到当前字符串的末尾。 |

### ReturnValue

自身引用。

## 另见

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::operator+=(uint64_t) method


连接赋值运算符。

```cpp
String & System::String::operator+=(uint64_t value)
```


| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 值 | uint64_t | Unsigned long 用于添加到当前字符串的末尾。 |

### ReturnValue

自身引用。

## 另见

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::operator+=(uint8_t) method


连接赋值运算符。

```cpp
String & System::String::operator+=(uint8_t value)
```


| 参数 | 类型 | 描述 |
| --- | --- | --- |
| value | uint8_t | [Byte](../../byte/) 用于添加到当前字符串的末尾。 |

### ReturnValue

自身引用。

## 另见

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)

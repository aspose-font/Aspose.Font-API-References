...
description: "System::setter_post_decrement_wrap 方法。翻译器将 C# 的后递减表达式（针对具有 setter 和 getter 定义的实例属性）转换为在 C++ 中调用此函数（针对 const getter 的重载）。"
type: docs
weight: 37700
...
翻译器将 C# 的后递减表达式（针对具有 setter 和 getter 定义的实例属性）转换为调用此函数（针对 const getter 的重载）。
...
翻译器将 C# 的后递减表达式（针对具有 setter 和 getter 定义的实例属性）转换为调用此函数（针对非 const getter 的重载）。
...
翻译器将 C# 的后递减表达式（针对具有 setter 和 getter 定义的类属性）转换为调用此函数。

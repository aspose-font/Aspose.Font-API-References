...
description: "System::setter_decrement_wrap 方法。翻译器将 C# 的前递减表达式（目标为具有已定义 setter 和 getter 的实例属性）转换为在 C++ 中调用此函数（针对 const getter 的重载）。"
type: docs
weight: 37200
...
翻译器将 C# 的前递减表达式（目标为具有已定义 setter 和 getter 的实例属性）转换为调用此函数（针对 const getter 的重载）。
...
翻译器将 C# 的前递减表达式（针对具有 setter 和 getter 定义的实例属性）转换为对该函数的调用（针对非常量 getter 的重载）。
...
翻译器将 C# 的前递减表达式（针对具有 setter 和 getter 定义的类属性）转换为对该函数的调用。

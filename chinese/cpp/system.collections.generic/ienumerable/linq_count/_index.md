---
title: "System::Collections::Generic::IEnumerable::LINQ_Count method"
linktitle: "LINQ_Count"
second_title: "Aspose.Font 适用于 C++"
description: "System::Collections::Generic::IEnumerable::LINQ_Count 方法。返回序列中元素的数量（通过直接计数计算），在 C++ 中。"
type: docs
weight: 1200
url: /zh/cpp/system.collections.generic/ienumerable/linq_count/
---
## IEnumerable::LINQ_Count() method


返回序列中元素的数量（通过直接计数计算）。

```cpp
int System::Collections::Generic::IEnumerable<T>::LINQ_Count()
```


### ReturnValue

序列中元素的数量。

## 另见

* Class [IEnumerable](../)
* Namespace [System::Collections::Generic](../../)
* Library [Aspose.Font for C++](../../../)
## IEnumerable::LINQ_Count(const Func\<T, bool\>\&) method


返回满足指定条件的序列中元素的数量。

```cpp
int System::Collections::Generic::IEnumerable<T>::LINQ_Count(const Func<T, bool> &predicate)
```


| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 谓词 | const Func\<T, bool\>\& | 用于测试每个元素是否满足条件的函数。 |

### ReturnValue

满足指定条件的序列中元素的数量。

## 另见

* Class [Func](../../../system/func/)
* Class [IEnumerable](../)
* Namespace [System::Collections::Generic](../../)
* Library [Aspose.Font for C++](../../../)

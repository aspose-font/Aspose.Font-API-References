...
description: "GlyphRendererBase 方法。使用独立的 glyph 路径渲染 glyph。RenderGlyph 函数族在渲染时会更改 glyph 路径。这导致需要再次重新加载该 glyph。此函数使用 glyph 路径的副本，不会更改原始 glyph 路径，因此同一 glyph 可以多次重复使用。此函数版本旨在与 glyph 缓存一起使用。"
type: docs
weight: 20

---
title: "طريقة System::TimeZoneInfo::ConvertTimeBySystemTimeZoneId"
linktitle: "ConvertTimeBySystemTimeZoneId"
second_title: "Aspose.Font لـ C++"
description: "طريقة System::TimeZoneInfo::ConvertTimeBySystemTimeZoneId. يحول الوقت إلى الوقت في منطقة زمنية محددة في C++."
type: docs
weight: 300
url: /ar/cpp/system/timezoneinfo/converttimebysystemtimezoneid/
---
## TimeZoneInfo::ConvertTimeBySystemTimeZoneId(const DateTimeOffset\&, const String\&) method


[Convert](../../convert/) time to the time in a specified time zone.

```cpp
static DateTimeOffset System::TimeZoneInfo::ConvertTimeBySystemTimeZoneId(const DateTimeOffset &date_time_offset, const String &destination_time_zone_id)
```


| معامل | نوع | الوصف |
| --- | --- | --- |
| date_time_offset | const DateTimeOffset\& | التاريخ والوقت للتحويل. |
| destination_time_zone_id | const String\& | معرف المنطقة الزمنية الوجهة. |

### ReturnValue

التاريخ والوقت المحوَّل.

## انظر أيضًا

* Class [DateTimeOffset](../../datetimeoffset/)
* Class [String](../../string/)
* Class [TimeZoneInfo](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## TimeZoneInfo::ConvertTimeBySystemTimeZoneId(DateTime, const String\&) method


[Convert](../../convert/) time to the time in a specified time zone.

```cpp
static DateTime System::TimeZoneInfo::ConvertTimeBySystemTimeZoneId(DateTime date_time, const String &destination_time_zone_id)
```


| معامل | نوع | الوصف |
| --- | --- | --- |
| date_time | DateTime | التاريخ والوقت للتحويل. |
| destination_time_zone_id | const String\& | معرف المنطقة الزمنية الوجهة. |

### ReturnValue

التاريخ والوقت المحوَّل.

## انظر أيضًا

* Class [DateTime](../../datetime/)
* Class [String](../../string/)
* Class [TimeZoneInfo](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## TimeZoneInfo::ConvertTimeBySystemTimeZoneId(DateTime, const String\&, const String\&) method


[Convert](../../convert/) time to the time in a specified time zone.

```cpp
static DateTime System::TimeZoneInfo::ConvertTimeBySystemTimeZoneId(DateTime date_time, const String &source_time_zone_id, const String &destination_time_zone_id)
```


| معامل | نوع | الوصف |
| --- | --- | --- |
| date_time | DateTime | التاريخ والوقت للتحويل. |
| source_time_zone_id | const String\& | معرّف المنطقة الزمنية المصدر. |
| destination_time_zone_id | const String\& | معرف المنطقة الزمنية الوجهة. |

### ReturnValue

التاريخ والوقت المحوَّل.

## انظر أيضًا

* Class [DateTime](../../datetime/)
* Class [String](../../string/)
* Class [TimeZoneInfo](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)

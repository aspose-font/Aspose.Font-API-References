---
title: "System::Globalization::CultureInfo::CultureInfo منشئ"
linktitle: "CultureInfo"
second_title: "Aspose.Font لـ C++"
description: "System::Globalization::CultureInfo::CultureInfo منشئ. المنشئ في C++."
type: docs
weight: 100
url: /ar/cpp/system.globalization/cultureinfo/cultureinfo/
---
## CultureInfo::CultureInfo(const String\&) constructor


المُنشئ.

```cpp
System::Globalization::CultureInfo::CultureInfo(const String &name)
```


| معامل | نوع | الوصف |
| --- | --- | --- |
| name | const String\& | اسم الثقافة. |

## انظر أيضًا

* Class [String](../../../system/string/)
* Class [CultureInfo](../)
* Namespace [System::Globalization](../../)
* Library [Aspose.Font for C++](../../../)
## CultureInfo::CultureInfo(const String\&, bool) constructor


المُنشئ.

```cpp
System::Globalization::CultureInfo::CultureInfo(const String &name, bool use_user_override)
```


| معامل | نوع | الوصف |
| --- | --- | --- |
| name | const String\& | اسم الثقافة. |
| use_user_override | bool | إذا كان صحيحًا، استخدم القيم المعرفة من قبل المستخدم بدلاً من القيم النظامية؛ تم تجاهل المعامل. |

## انظر أيضًا

* Class [String](../../../system/string/)
* Class [CultureInfo](../)
* Namespace [System::Globalization](../../)
* Library [Aspose.Font for C++](../../../)
## CultureInfo::CultureInfo(int) constructor


معلومات RTTI.

```cpp
System::Globalization::CultureInfo::CultureInfo(int culture)
```


| معامل | نوع | الوصف |
| --- | --- | --- |
| الثقافة | int | معرف الثقافة. |
## ملاحظات


المُنشئ.
## انظر أيضًا

* Class [CultureInfo](../)
* Namespace [System::Globalization](../../)
* Library [Aspose.Font for C++](../../../)
## CultureInfo::CultureInfo(int, bool) constructor


المُنشئ.

```cpp
System::Globalization::CultureInfo::CultureInfo(int culture, bool use_user_override)
```


| معامل | نوع | الوصف |
| --- | --- | --- |
| الثقافة | int | معرف الثقافة. |
| use_user_override | bool | إذا كان صحيحًا، استخدم القيم المعرفة من قبل المستخدم بدلاً من القيم النظامية؛ تم تجاهل المعامل. |

## انظر أيضًا

* Class [CultureInfo](../)
* Namespace [System::Globalization](../../)
* Library [Aspose.Font for C++](../../../)
## CultureInfo::CultureInfo(std::nullptr_t) constructor


دائمًا يرمي استثناء ArgumentNullException.

```cpp
System::Globalization::CultureInfo::CultureInfo(std::nullptr_t)
```

## انظر أيضًا

* Class [CultureInfo](../)
* Namespace [System::Globalization](../../)
* Library [Aspose.Font for C++](../../../)

---
title: "Espacio de nombres System::Threading::Tasks"
linktitle: "System::Threading::Tasks"
second_title: "Aspose.Font para C++"
description: "Cómo usar el espacio de nombres System::Threading::Tasks en C++."
type: docs
weight: 7000
url: /es/cpp/system.threading.tasks/
---



## Clases

| Clase | Descripción |
| --- | --- |
| [Parallel](./parallel/) | Proporciona soporte para bucles paralelos y regiones. |
| [ParallelLoopResult](./parallelloopresult/) | Proporciona el estado de finalización de un bucle [Parallel](./parallel/). |
| [ParallelOptions](./paralleloptions/) | Almacena opciones que configuran el funcionamiento de los métodos en la clase [Parallel](./parallel/). |
| [ResultTask](./resulttask/) | Una especialización de [Task](./task/) que devuelve un valor de resultado al completarse. |
| [ResultValueTask](./resultvaluetask/) | Representa un tipo híbrido similar a una tarea que puede envolver ya sea un valor de resultado directo o un [ResultTask<T>](./resulttask/). |
| [Task](./task/) | Representa una operación asincrónica que puede esperarse y combinarse con otras tareas. |
| [TaskScheduler](./taskscheduler/) | Representa un objeto que maneja el trabajo de bajo nivel de encolar tareas en hilos. |
| [ValueTask](./valuetask/) | Proporciona un resultado esperable de una operación asincrónica. |
## Enums

| Enumeración | Descripción |
| --- | --- |
| [TaskStatus](./taskstatus/) |  |
## Functions

| Función | Descripción |
| --- | --- |
| Delay | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| Delay | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| FromCanceled | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| FromException | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| FromException | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| FromResult | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| Run | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| Run | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| Run | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| Run | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| WaitAll | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| WaitAll | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| WaitAny | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| WaitAny | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| WhenAll | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| WhenAll | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| WhenAll | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| WhenAll | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| WhenAny | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| WhenAny | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| WhenAny | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| WhenAny | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |
| Yield | System.Collections.Generic.List`1[Doxygen2HugoConverter.Markup.SimpleMarkupEntry] |

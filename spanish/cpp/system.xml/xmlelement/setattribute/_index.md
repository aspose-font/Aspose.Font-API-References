---
title: "Método System::Xml::XmlElement::SetAttribute"
linktitle: "SetAttribute"
second_title: "Aspose.Font para C++"
description: "Método System::Xml::XmlElement::SetAttribute. Establece el valor del atributo con el nombre local y el URI de espacio de nombres especificados en C++."
type: docs
weight: 2600
url: /es/cpp/system.xml/xmlelement/setattribute/
---
## XmlElement::SetAttribute(String, String, String) method


Establece el valor del atributo con el nombre local y el URI del espacio de nombres especificados.

```cpp
virtual String System::Xml::XmlElement::SetAttribute(String localName, String namespaceURI, String value)
```


| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| localName | Cadena | El nombre local del atributo. |
| namespaceURI | Cadena | El URI del espacio de nombres del atributo. |
| valor | Cadena | El valor a establecer para el atributo. |

### ReturnValue

El valor del atributo.

## Ver también

* Class [String](../../../system/string/)
* Class [XmlElement](../)
* Namespace [System::Xml](../../)
* Library [Aspose.Font for C++](../../../)
## XmlElement::SetAttribute(String, String) method


Establece el valor del atributo con el nombre especificado.

```cpp
virtual void System::Xml::XmlElement::SetAttribute(String name, String value)
```


| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| nombre | Cadena | El nombre del atributo a crear o modificar. Este es un nombre calificado. Si el nombre contiene dos puntos, se analiza en componentes de prefijo y nombre local. |
| valor | Cadena | El valor a establecer para el atributo. |

## Ver también

* Class [String](../../../system/string/)
* Class [XmlElement](../)
* Namespace [System::Xml](../../)
* Library [Aspose.Font for C++](../../../)

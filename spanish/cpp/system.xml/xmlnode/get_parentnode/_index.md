...
Si un nodo acaba de ser creado y aún no se ha añadido al árbol, o si ha sido eliminado del árbol, el padre es **nullptr**. Para todos los demás nodos, el valor devuelto depende de [XmlNode::get_NodeType](../get_nodetype/) del nodo. La tabla siguiente describe los posibles valores de retorno para el método **get_NodeType**. |||
|-|-|
...
| Comentario | Devuelve el elemento, la referencia de entidad, el tipo de documento o el documento que contiene el comentario. |
| TipoDeDocumento | Devuelve el nodo documento. |
| Elemento | Devuelve el nodo padre del elemento. Si el elemento es el nodo raíz del árbol, el padre es el nodo documento. |
| EntityReference | Devuelve el elemento, el atributo o la referencia de entidad que contiene la referencia de entidad. |
| InstrucciónDeProcesamiento | Devuelve el documento, elemento, tipo de documento o referencia de entidad que contiene la instrucción de procesamiento. |

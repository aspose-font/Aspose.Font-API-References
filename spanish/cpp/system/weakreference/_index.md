---
title: "Clase System::WeakReference"
linktitle: "WeakReference"
second_title: "Aspose.Font para C++"
description: "Clase System::WeakReference. Representa una referencia débil, que referencia a un objeto mientras aún permite que ese objeto sea eliminado en C++."
type: docs
weight: 7600
url: /es/cpp/system/weakreference/
---
## WeakReference class


Representa una referencia débil, que referencia un objeto mientras aún permite que ese objeto sea eliminado.

```cpp
template<typename...>class WeakReference
```


| Parámetro | Descripción |
| --- | --- |
| Argumentos | Argumentos de plantilla de una referencia débil que declaran el tipo de un objeto referenciado; el número de argumentos puede ser cero o uno. |

## Ver también

* Namespace [System](../)
* Library [Aspose.Font for C++](../../)

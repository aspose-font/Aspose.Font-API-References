---
title: "System::Nullable::Nullable constructor"
linktitle: "Nullable"
second_title: "Aspose.Font para C++"
description: "System::Nullable::Nullable constructor. Construye una instancia que representa un valor nulo en C++."
type: docs
weight: 100
url: /es/cpp/system/nullable/nullable/
---
## Nullable::Nullable() constructor


Construye una instancia que representa un valor nulo.

```cpp
System::Nullable<T>::Nullable()
```

## Ver también

* Class [Nullable](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## Nullable::Nullable(const Nullable\<T1\>\&) constructor


Construye una instancia que representa un valor que es representado por el objeto [Nullable](../) especificado. El objeto nullable especificado puede representar un valor de tipo diferente al tipo subyacente de la instancia construida, en cuyo caso el valor representado se convierte a un valor de tipo T.

```cpp
template<typename T1> System::Nullable<T>::Nullable(const Nullable<T1> &value)
```


| Parámetro | Descripción |
| --- | --- |
| T1 | El tipo del valor representado por el objeto [Nullable](../) especificado |

## Ver también

* Class [Nullable](../)
* Class [Nullable](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## Nullable::Nullable(const T1\&) constructor


Construye una instancia de la clase [Nullable](../) que representa el valor especificado convertido (si es necesario) al valor del tipo subyacente T.

```cpp
template<typename T1> System::Nullable<T>::Nullable(const T1 &value)
```


| Parámetro | Descripción |
| --- | --- |
| T1 | El tipo del valor especificado |

| Parámetro | Tipo | Descripción |
| --- | --- | --- |
| value | const T1\& | Una referencia constante al valor que será representado por el objeto [Nullable](../) recién construido |

## Ver también

* Class [Nullable](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## Nullable::Nullable(std::nullptr_t) constructor


Construye una instancia que representa nulo.

```cpp
System::Nullable<T>::Nullable(std::nullptr_t)
```

## Ver también

* Class [Nullable](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)

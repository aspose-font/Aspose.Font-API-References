...
description: "System::IO::BasicSystemIOStreamBuf class. Представляет буфер, который оборачивает потоки, похожие на System::IO::Stream, и позволяет использовать их в качестве внутреннего буфера потоков, аналогичного std::iostream, в C++."
type: docs
weight: 400
...
Представляет буфер, который оборачивает [System::IO::Stream](../stream/)-подобные потоки и позволяет использовать их в качестве внутреннего буфера, аналогичного std::iostream.
...
| [operator=](./operator=/)(const BasicSystemIOStreamBuf\&) | Оператор копирующего присваивания. Удалён. |

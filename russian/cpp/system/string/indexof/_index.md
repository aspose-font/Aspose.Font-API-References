---
title: "Метод System::String::IndexOf"
linktitle: "IndexOf"
second_title: "Aspose.Font для C++"
description: "Метод System::String::IndexOf. Поиск символа вперёд в подстроке в C++."
type: docs
weight: 1500
url: /ru/cpp/system/string/indexof/
---
## String::IndexOf(char_t, int, int) const method


Поиск символа вперёд в подстроке.

```cpp
int System::String::IndexOf(char_t c, int startIndex, int count) const
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| c | char_t | Символ для поиска. |
| startIndex | int | Индекс, с которого начинать поиск. |
| count | int | Количество символов для просмотра. |

### ReturnValue

Индекс первой позиции символа, начиная с startIndex, или -1, если не найден.

## См. также

* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::IndexOf(char_t, int) const method


Поиск символа вперёд.

```cpp
int System::String::IndexOf(char_t c, int startIndex=0) const
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| c | char_t | Символ для поиска. |
| startIndex | int | Индекс, с которого начинать поиск. |

### ReturnValue

Индекс первой позиции символа, начиная с startIndex, или -1, если не найден.

## См. также

* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::IndexOf(const String\&, int, int) const method


Прямой поиск подстроки.

```cpp
int System::String::IndexOf(const String &str, int startIndex, int count) const
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| str | const String\& | Подстрока для поиска. |
| startIndex | int | Позиция в исходной строке, с которой начинать поиск. |
| count | int | количество символов для просмотра. |

### ReturnValue

Индекс первого найденного подстрока или -1, если не найдено. Для пустой строки поиска всегда возвращает startIndex.

## См. также

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::IndexOf(const String\&, int, System::StringComparison) const method


Прямой поиск подстроки.

```cpp
int System::String::IndexOf(const String &str, int startIndex, System::StringComparison comparison_type) const
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| str | const String\& | Подстрока для поиска. |
| startIndex | int | Позиция в исходной строке, с которой начинать поиск. |
| comparison_type | System::StringComparison | [Comparison](../../comparison/) режим. |

### ReturnValue

Индекс первого найденного подстрока или -1, если не найдено. Для пустой строки поиска всегда возвращает startIndex.

## См. также

* Class [String](../)
* Enum [StringComparison](../../stringcomparison/)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::IndexOf(const String\&, int) const method


Прямой поиск подстроки.

```cpp
int System::String::IndexOf(const String &str, int startIndex=0) const
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| str | const String\& | Подстрока для поиска. |
| startIndex | int | Позиция в исходной строке, с которой начинать поиск. |

### ReturnValue

Индекс первого найденного подстрока или -1, если не найдено. Для пустой строки поиска всегда возвращает startIndex.

## См. также

* Class [String](../)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::IndexOf(const String\&, System::StringComparison) const method


Прямой поиск подстроки.

```cpp
int System::String::IndexOf(const String &str, System::StringComparison comparison_type) const
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| str | const String\& | Подстрока для поиска. |
| comparison_type | System::StringComparison | [Comparison](../../comparison/) режим. |

### ReturnValue

Индекс первого найденного подстрока или -1, если не найдено. Для пустой строки поиска всегда возвращает 0.

## См. также

* Class [String](../)
* Enum [StringComparison](../../stringcomparison/)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)
## String::IndexOf(const String\&, int, int, System::StringComparison) const method


Прямой поиск подстроки.

```cpp
int System::String::IndexOf(const String &value, int startIndex, int count, System::StringComparison comparisonType) const
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| значение | const String\& | Подстрока для поиска. |
| startIndex | int | Позиция в исходной строке, с которой начинать поиск. |
| count | int | количество символов для просмотра. |
| comparisonType | System::StringComparison | [Comparison](../../comparison/) режим. |

### ReturnValue

Индекс первого найденного подстрока или -1, если не найдено. Для пустой строки поиска всегда возвращает startIndex.

## См. также

* Class [String](../)
* Enum [StringComparison](../../stringcomparison/)
* Class [String](../)
* Namespace [System](../../)
* Library [Aspose.Font for C++](../../../)

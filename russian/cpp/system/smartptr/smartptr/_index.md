...
| --- | --- | --- |
| src | const SmartPtr\<Array\<Y\>\>\& | Указатель на массив, из которого нужно создать копию, но с другим типом элементов. |
...
| Параметр | Тип | Описание |
| --- | --- | --- |
| ptr | const SmartPtr\<P\>\& | Другой умный указатель для совместного владения с тем, откуда. |
...
Копирующий конструктор объекта [SmartPtr](../). Оба указателя указывают на один и тот же объект после этого. Выполняет преобразование типа, если это разрешено.
...
Копирующий конструктор объекта [SmartPtr](../). Оба указателя указывают на один и тот же объект после этого.
...
Перемещающий конструктор объекта [SmartPtr](../). По сути, меняет местами два указателя, если они оба в одном режиме. x может стать недоступным после вызова.

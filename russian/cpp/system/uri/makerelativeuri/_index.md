...
Если имя хоста и схема URI, представленных текущим объектом и **toUri**, одинаковы, то этот метод возвращает относительный [Uri](../), который при добавлении к текущему экземпляру URI дает **toUri**. Если имя хоста или схема различаются, то метод возвращает объект [Uri](../), представляющий параметр **uri**.

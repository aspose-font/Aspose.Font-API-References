...
description: "Класс System::Xml::XmlNode. Представляет отдельный узел в XML‑документе на C++."
type: docs
weight: 2500
...
| virtual [CloneNode](./clonenode/)(bool) | Создаёт дубликат узла, когда переопределяется в производном классе. |
...
| virtual [get_InnerText](./get_innertext/)() | Возвращает объединённые значения узла и всех его дочерних узлов. |
...
| virtual [get_LocalName](./get_localname/)() | Возвращает локальное имя узла, когда переопределяется в производном классе. |
| virtual [get_Name](./get_name/)() | Возвращает полное (квалифицированное) имя узла, когда переопределяется в производном классе. |
...
| virtual [get_NodeType](./get_nodetype/)() | Возвращает тип текущего узла, когда переопределяется в производном классе. |
...
| virtual [GetNamespaceOfPrefix](./getnamespaceofprefix/)(String) | Ищет ближайшее объявление **xmlns** для указанного префикса, которое находится в области действия текущего узла, и возвращает URI пространства имён в объявлении. |
| virtual [GetPrefixOfNamespace](./getprefixofnamespace/)(String) | Ищет ближайшее объявление **xmlns** для указанного URI пространства имён, которое находится в области действия текущего узла, и возвращает префикс, определённый в этом объявлении. |
| virtual [idx_get](./idx_get/)(String) | Возвращает первый дочерний элемент с указанным [XmlNode::get_Name](./get_name/). |
| virtual [idx_get](./idx_get/)(String, String) | Возвращает первый дочерний элемент с указанными значениями [XmlNode::get_LocalName](./get_localname/) и [XmlNode::get_NamespaceURI](./get_namespaceuri/). |
...
| virtual [Normalize](./normalize/)() | Помещает все узлы [XmlText](../xmltext/) на полной глубине поддерева под этим [XmlNode](./) в «нормальную» форму, где только разметка (т.е. теги, комментарии, инструкции обработки, секции CDATA и ссылки на сущности) разделяют узлы [XmlText](../xmltext/), то есть соседних узлов [XmlText](../xmltext/) нет. |

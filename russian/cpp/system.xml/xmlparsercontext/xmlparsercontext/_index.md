...
| --- | --- | --- |
| nt | const SharedPtr\<XmlNameTable\>\& | Таблица [XmlNameTable](../../xmlnametable/) используется для атомизации строк. Если это **nullptr**, используется таблица имён, применённая при построении **nsMgr**. Для получения дополнительной информации об атомизированных строках см. [XmlNameTable](../../xmlnametable/). |
| nsMgr | const SharedPtr\<XmlNamespaceManager\>\& | [XmlNamespaceManager](../../xmlnamespacemanager/) используется для поиска информации о пространствах имён, или **nullptr**. |
...
| baseURI | const String\& | Базовый URI для XML‑фрагмента (место, из которого был загружен фрагмент). |
| xmlLang | const String\& | Область действия **xml:lang**. |

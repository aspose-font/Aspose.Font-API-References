...
| --- | --- | --- |
| buffer | const ArrayPtr\<char16_t\>\& | Массив символов, который служит буфером, в который записывается текстовое содержимое. |
| индекс | int32_t | Позиция в **buffer**, с которой метод может начать запись текстового содержимого. |
| count | int32_t | Количество символов для записи в **buffer**. |

### ReturnValue

Количество прочитанных символов. Может быть 0, если читатель не находится на элементе или если в текущем контексте больше нет текстового содержимого для возврата.

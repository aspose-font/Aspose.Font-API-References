---
title: "Метод System::Drawing::Graphics::DrawImageUnscaled"
linktitle: "DrawImageUnscaled"
second_title: "Aspose.Font для C++"
description: "Метод System::Drawing::Graphics::DrawImageUnscaled. Рисует указанное изображение, используя его оригинальный физический размер, в указанном месте в C++."
type: docs
weight: 1900
url: /ru/cpp/system.drawing/graphics/drawimageunscaled/
---
## Graphics::DrawImageUnscaled(const SharedPtr\<Image\>\&, const Point\&) method


Рисует указанное изображение, используя его оригинальный физический размер, в указанном месте.

```cpp
void System::Drawing::Graphics::DrawImageUnscaled(const SharedPtr<Image> &image, const Point &point)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| image | const SharedPtr\<Image\>\& | Изображение для рисования |
| point | const Point\& | Структура [Point](../../point/), определяющая левый верхний угол рисуемого изображения. |

## См. также

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [Image](../../image/)
* Class [Point](../../point/)
* Class [Graphics](../)
* Namespace [System::Drawing](../../)
* Library [Aspose.Font for C++](../../../)
## Graphics::DrawImageUnscaled(const SharedPtr\<Image\>\&, const Rectangle\&) method


Рисует указанное изображение, используя его оригинальный физический размер, в указанном месте.

```cpp
void System::Drawing::Graphics::DrawImageUnscaled(const SharedPtr<Image> &image, const Rectangle &rect)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| image | const SharedPtr\<Image\>\& | Изображение для рисования |
| rect | const Rectangle\& | Прямоугольник, определяющий левый верхний угол рисуемого изображения. Свойства X и Y прямоугольника задают левый верхний угол. Значения ширины и высоты игнорируются. |

## См. также

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [Image](../../image/)
* Class [Rectangle](../../rectangle/)
* Class [Graphics](../)
* Namespace [System::Drawing](../../)
* Library [Aspose.Font for C++](../../../)
## Graphics::DrawImageUnscaled(const SharedPtr\<Image\>\&, int, int) method


Рисует указанное изображение, используя его оригинальный физический размер, в указанном месте.

```cpp
void System::Drawing::Graphics::DrawImageUnscaled(const SharedPtr<Image> &image, int x, int y)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| image | const SharedPtr\<Image\>\& | Изображение для рисования |
| x | int | Координата X левого верхнего угла рисуемого изображения |
| y | int | Координата Y левого верхнего угла рисуемого изображения |

## См. также

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [Image](../../image/)
* Class [Graphics](../)
* Namespace [System::Drawing](../../)
* Library [Aspose.Font for C++](../../../)
## Graphics::DrawImageUnscaled(const SharedPtr\<Image\>\&, int, int, int, int) method


Рисует указанное изображение, используя его оригинальный физический размер, в указанном месте.

```cpp
void System::Drawing::Graphics::DrawImageUnscaled(const SharedPtr<Image> &image, int x, int y, int width, int height)
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| image | const SharedPtr\<Image\>\& | Изображение для рисования |
| x | int | Координата X левого верхнего угла рисуемого изображения |
| y | int | Координата Y левого верхнего угла рисуемого изображения |
| width | int | Не используется |
| height | int | Не используется |

## См. также

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [Image](../../image/)
* Class [Graphics](../)
* Namespace [System::Drawing](../../)
* Library [Aspose.Font for C++](../../../)

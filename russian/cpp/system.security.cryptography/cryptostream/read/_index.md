---
title: "Метод System::Security::Cryptography::CryptoStream::Read"
linktitle: "Read"
second_title: "Aspose.Font для C++"
description: "Метод System::Security::Cryptography::CryptoStream::Read. Считывает данные из потока в C++."
type: docs
weight: 1000
url: /ru/cpp/system.security.cryptography/cryptostream/read/
---
## CryptoStream::Read(const ArrayPtr\<uint8_t\>\&, int32_t, int32_t) method


Читает данные из потока.

```cpp
int32_t System::Security::Cryptography::CryptoStream::Read(const ArrayPtr<uint8_t> &buffer, int32_t offset, int32_t count) override
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| buffer | const ArrayPtr\<uint8_t\>\& | Буфер данных назначения. |
| смещение | int32_t | Смещение в буфере назначения. |
| count | int32_t | Количество байтов для чтения. |

### ReturnValue

Фактическое количество читаемых байтов.

## См. также

* Typedef [ArrayPtr](../../../system/arrayptr/)
* Class [CryptoStream](../)
* Namespace [System::Security::Cryptography](../../)
* Library [Aspose.Font for C++](../../../)
## CryptoStream::Read(const System::Details::ArrayView\<uint8_t\>\&, int32_t, int32_t) method


Читает данные из потока.

```cpp
int32_t System::Security::Cryptography::CryptoStream::Read(const System::Details::ArrayView<uint8_t> &buffer, int32_t offset, int32_t count) override
```


| Параметр | Тип | Описание |
| --- | --- | --- |
| buffer | const System::Details::ArrayView\<uint8_t\>\& | Буфер данных назначения. |
| смещение | int32_t | Смещение в буфере назначения. |
| count | int32_t | Количество байтов для чтения. |

### ReturnValue

Фактическое количество читаемых байтов.

## См. также

* Class [CryptoStream](../)
* Namespace [System::Security::Cryptography](../../)
* Library [Aspose.Font for C++](../../../)

---
title: "System::DynamicWeakPtr::Reference::Reference constructor"
linktitle: "Reference"
second_title: "Aspose.Font için C++"
description: "System::DynamicWeakPtr::Reference::Reference constructor. C++'ta akıllı işaretçi referansı oluşturur."
type: docs
weight: 100
url: /tr/cpp/system/dynamicweakptr/reference/reference/
---
## Reference::Reference(DynamicWeakPtr_\&) constructor


Akıllı gösterici referansı oluşturur.

```cpp
System::DynamicWeakPtr<T, trunkMode, weakLeafs>::Reference::Reference(DynamicWeakPtr_ &ptr)
```


| Parametre | Tür | Açıklama |
| --- | --- | --- |
| ptr | DynamicWeakPtr_\& | Akıllı işaretçiye ham referans. |

## Ayrıca Bakınız

* Typedef [DynamicWeakPtr_](../../dynamicweakptr_/)
* Class [Reference](../)
* Class [DynamicWeakPtr](../../)
* Namespace [System](../../../)
* Library [Aspose.Font for C++](../../../../)
## Reference::Reference(Reference\&&) constructor


Akıllı gösterici referansını taşıyarak oluşturur.

```cpp
System::DynamicWeakPtr<T, trunkMode, weakLeafs>::Reference::Reference(Reference &&reference) noexcept
```


| Parametre | Tür | Açıklama |
| --- | --- | --- |
| reference | Reference\&& | [Reference](../) taşımak için. |

## Ayrıca Bakınız

* Class [Reference](../)
* Class [Reference](../)
* Class [DynamicWeakPtr](../../)
* Namespace [System](../../../)
* Library [Aspose.Font for C++](../../../../)

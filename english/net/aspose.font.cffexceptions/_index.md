---
title: Aspose.Font.CffExceptions
second_title: Aspose.Font for .NET API Reference
description: The **Aspose.Font.CffExceptions** namespace provides classes describing exceptions related to Compact Font Format.
type: docs
weight: 30
url: /net/aspose.font.cffexceptions/
---
The **Aspose.Font.CffExceptions** namespace provides classes describing exceptions related to Compact Font Format.

## Classes

| Class | Description |
| --- | --- |
| class [CffFontException](./cfffontexception) | Represents common processing related exception for fonts of CFF format. |
| class [CffParsingException](./cffparsingexception) | Represents parse exception for fonts of cff format. The exception can be thrown in case errors during font parsing process |

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Font.dll -->
